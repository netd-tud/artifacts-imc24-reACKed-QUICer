...
def conv_to_file_size(size: str):
    if "KB" in size:
        size = int(size.split("KB")[0]) * KB
    else:
        size = int(size.split("MB")[0]) * MB
    return size
...
class TestCase(abc.ABC):
    ...
    def __init__(
        ...
        self._drop_to_server = drop_to_server
        self._drop_to_client = drop_to_client
    ...
    def scenario(self) -> str:
        ...
        return f"simple-p2p --delay={self._default_delay} --bandwidth={self._bandwidth} --queue={self._queue}"
    ...
    def certs_dir(self):
        if not self._cert_dir:
            ...
            generate_cert_chain(self._cert_dir.name, int(self._cert_chain))
...
class TestCaseTransferDroplist(TestCaseTransfer):
    @staticmethod
    def name():
        return "transferdroplist"
    ...
    @staticmethod
    def desc():
        return "Stream data is being sent and received correctly. Connection close completes with a zero error code. But with packet drops"

    def scenario(self):
        """Scenario for the ns3 simulator"""
        scen_text = f"droplist --delay={self._default_delay} --bandwidth={self._bandwidth} --queue={self._queue}"
        if len(self._drop_to_client) > 0:
            scen_text += f" --drops_to_client={self._drop_to_client}"

        if len(self._drop_to_server) > 0:
            scen_text += f" --drops_to_server={self._drop_to_server}"
...
class TestCaseHTTP3c(TestCase):
    ...
    def testname(self, p: Perspective):
        # Transmit http3 to docker container
        return  "http3"
    ...
    def get_paths(self):
        self.FILESIZE = conv_to_file_size(self._file_size)
...
class TestCaseHTTP3cDroplist(TestCaseHTTP3c):

    @staticmethod
    def name():
        return "http3cdroplist"
    ...
    @staticmethod
    def desc():
        return "An H3 transaction under packet loss"
...
class MeasurementGoodputDroplist(MeasurementGoodput):

    @staticmethod
    def name():
        return "goodputdroplist"
    ...
    @staticmethod
    def desc():
        return "Measures connection goodput over a 10Mbps link.. But with packet drops"
