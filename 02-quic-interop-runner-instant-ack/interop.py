...
class InteropRunner:
    ...
    _queue = "25"
    _enable_instant_ack = ("",)
    _enable_instant_ack_delay = ("",)
    _enable_first_packet_delay = ("",)
    _file_size = ("",)
    _repetitions = ("100",)
    _cert_chain = ("1",)
    _enable_qlog = (True,)
    ...
    def __init__(
        ...
        queue="25",
        enable_instant_ack="",
        enable_instant_ack_delay="",
        enable_first_packet_delay="",
        repetitions=100,
        pcaps_disable=False,
        file_size="10",
        cert_chain="1",
        ...
        self._queue = queue
        self._enable_instant_ack = enable_instant_ack
        self._enable_instant_ack_delay = enable_instant_ack_delay
        self._enable_first_packet_delay = enable_first_packet_delay
        self._repetitions = int(repetitions)
        self._pcaps_disable = pcaps_disable
        self._file_size = file_size
        self._cert_chain = cert_chain
        self._enable_qlog = enable_qlog

        if len(custom_name) > 0:
            custom_name = f"_{custom_name}"
        if len(self._log_dir) == 0:
            self._log_dir = f"logs{custom_name}" + "_{:%Y-%m-%dT%H:%M:%S}".format(
                self._start_time
            )
    ...
    def _check_impl_is_compliant(self, name: str) -> bool:
        ...
        qlogdir = "/logs/qlog/"
        if not self._enable_qlog:
            logging.debug("Disabled qlog.")
            qlogdir = ""
        ...
        cmd = (
            ...
            f"ENABLEINSTANTACK={self._enable_instant_ack} "
            f"ENABLEINSTANTACKDELAY={self._enable_instant_ack_delay} "
            f"ENABLEFIRSTPACKETDELAY={self._enable_first_packet_delay} "
            f"QLOGDIR={qlogdir} "
    ...
    def _run_testcase(
        ...
        return self._run_test(
            server, client, None if (repetition == "0") else repetition, test
        )[0]
    ...
    def _run_test(
        ...
        testcase = test(
            ...
            drop_to_server=self._drop_to_server,
            drop_to_client=self._drop_to_client,
            bandwidth=self._bandwidth,
            queue=self._queue,
            file_size=self._file_size,
            repetitions=self._repetitions,
            cert_chain=self._cert_chain,
        )
        print(
            "Server: "
            + server
        ...
        for server in self._servers:
            for client in self._clients:
                ...
                for testcase in self._tests:
                    if self._repetitions and self._repetitions > 0:
                        for i in range(0, self._repetitions):
                            status = self._run_testcase(
                                server, client, testcase, "%d" % (i + 1)
                            )
                            # Output only show last testcase result
                            self.test_results[server][client][testcase] = status
                            if status == TestResult.FAILED:
                                nr_failed += 1
                    else:
                        status = self._run_testcase(server, client, testcase, "0")
                        self.test_results[server][client][testcase] = status
                        if status == TestResult.FAILED:
                            nr_failed += 1
