...
func betterSleep(start time.Time, duration time.Duration) {
	for {
...
func (s *connection) handleUnpackedLongHeaderPacket(
	...
	if !s.receivedFirstPacket {
		...
		if s.perspective == protocol.PerspectiveServer {
			...
			if s.config.EnableFirstPacketDelay > 0 {
				start := time.Now()
				println("Start", start.String())
				print(s.config.EnableFirstPacketDelay)
				betterSleep(start, s.config.EnableFirstPacketDelay)
				println("stop", time.Now().String())
				print("slept")
			}
			if s.config.EnableInstantACK {
				s.receivedPacketHandler.ReceivedPacket(packet.hdr.PacketNumber, ecn, packet.encryptionLevel, rcvTime, true)
				s.triggerSending()
				instantACKSent = true
				// Busy wait
				start := time.Now()
				betterSleep(start, s.config.EnableInstantACKDelay)
				// This is imprecise.
				//time.Sleep(s.config.EnableInstantACKDelay)
			}
		}
	}
